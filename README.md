...
## 실험 결과
* 알고리즘을 여러 데이터 셋에 적용해본 결과, 대체로 원하는 좋은 결과를 얻을 수 있었다. 하지만 시도에 따라 결과가 달라지거나 무한 루프가 발생하여 결과를 도출하지 못하는 경우가 발생했다.
* 분포가 밀집된 데이터 셋에서 자주 무한 루프가 발생하였으며, 이상치 데이터를 클러스터에 포함하면서 오차가 발생하는 경우 또한 종종 있었다.

## 개선방안
* 실험 결과에서 나타난 문제 원인을 분석해본 결과, 알고리즘의 성능이 초기 클러스터 중심에 의해 크게 좌우됨을 확인할 수 있었다.
